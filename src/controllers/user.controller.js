...
const updatedata  = async (req,res)=>{
    try {
        ...
        if(result!==null){
            let result = await db.user.update({password:req.body.confirmpassword},{where:{email:data}});
            res.status(200).send({message:"your password has changed successfully"})
        }else{
